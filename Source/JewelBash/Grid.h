...
struct FTileType
{
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite)
	FTileSpecialities Specialities;
...
UCLASS()
class JEWELBASH_API AGrid : public AActor
{
	GENERATED_BODY()
	
public:	
	// Sets default values for this actor's properties
	AGrid(const FObjectInitializer& ObjectInitalizer);
	...
	UFUNCTION(BlueprintImplementableEvent, meta = (ExpandEnumAsExecs = "MoveType"), Category = Tile)
		void OnMoveMade(EJewelBashMoveType::Type MoveType);
	...
	void SetLastMove(EJewelBashMoveType::Type MoveType);

	/** Tells what type of move the specified player made most recently. */
	EJewelBashMoveType::Type GetLastMove();

	/** Gives point value per tile based on move type. Default is 100. */
	UFUNCTION(BlueprintNativeEvent, Category = Game)
		int32 GetScoreMultiplierForMove(EJewelBashMoveType::Type LastMoveType);
	virtual int32 GetScoreMultiplierForMove_Implementation(EJewelBashMoveType::Type LastMoveType);
...
private:
	...
	TMap<APlayerController*, EJewelBashMoveType::Type> LastMoves;
