...
void AGrid::RespawnTiles()
{
	...
	{
		SetLastMove(EJewelBashMoveType::MT_Combo);
	...
	else
	{
		if (IsUnwinnable())
		{
			if (AJewelBashGameMode* GameMode = Cast<AJewelBashGameMode>(UGameplayStatics::GetGameMode(this)))
			{
				GameMode->GameOver();
				return;
			}
		}
		UJewelBlueprintFunctionLibrary::PauseGameTimer(this, false);
	}
}
...
TArray<ATile*> AGrid::GetExplosionList(ATile* A) const
{
	...
	if (AJewelBashGameMode* GameMode = Cast<AJewelBashGameMode>(UGameplayStatics::GetGameMode(A)))
	{
...
void AGrid::ExecuteMatch(const TArray<ATile*>& MatchingTiles)
{
	...
	UJewelBlueprintFunctionLibrary::PauseGameTimer(this, true);
	...
	// Add score based on tile count.
	{
		if (AJewelBashGameMode* GameMode = Cast<AJewelBashGameMode>(UGameplayStatics::GetGameMode(this)))
		{
			EJewelBashMoveType::Type MT = GetLastMove();
			int32 ScoreMult = GetScoreMultiplierForMove(MT);
			// Special results for certain move types.
			switch (MT)
			{
			case EJewelBashMoveType::MT_Bomb:
			case EJewelBashMoveType::MT_AllTheBombs:
				// Clear combo when bombing.
				GameMode->SetComboPower(0);
				break;
			case EJewelBashMoveType::MT_Combo:
				// Power up combo!
...
void AGrid::OnSwapDisplayFinished(ATile* Tile)
{
	...
	if (SwappingTiles.Num() == 2)
	{
		...
		if (bPendingSwapMoveSuccess)
		{
			...
			{
				SetLastMove(EJewelBashMoveType::MT_MoreTiles);
			}
			else
			{
				SetLastMove(EJewelBashMoveType::MT_Standard);
			}
		...
		{
			SwappingTiles.Empty();
			OnMoveMade(EJewelBashMoveType::MT_Failure);
		}
	}
}
...
void AGrid::OnTileWasSelected(ATile* NewSelectedTile)
{
	...
	if (FallingTiles.Num() || TilesBeingDestroyed.Num() || bPendingSwapMove || !UJewelBlueprintFunctionLibrary::IsGameActive(this) || !NewSelectedTile)
	{
		return;
	}
		...
		if (NewSelectedTileType.Abilities.CanExplode())
		{
			...
			if (AJewelBashGameMode* GameMode = Cast<AJewelBashGameMode>(UGameplayStatics::GetGameMode(this)))
			{
				if (GameMode->GetComboPower() == GameMode->GetMaxComboPower())
				{
					...
					SetLastMove(EJewelBashMoveType::MT_AllTheBombs);
			...
			if (TilesToDestroy.Num() == 0)
			{
				...
				SetLastMove(EJewelBashMoveType::MT_Bomb);
...
void AGrid::SetLastMove(EJewelBashMoveType::Type MoveType)
{
	if (APlayerController* PC = UJewelBlueprintFunctionLibrary::GetLocalPlayerController(this))
	{
		...
		EJewelBashMoveType::Type& LastMoveType = LastMoves.FindOrAdd(PC);
		LastMoveType = MoveType;
	}
}

EJewelBashMoveType::Type AGrid::GetLastMove()
{
	...
	if (APlayerController* PC = UJewelBlueprintFunctionLibrary::GetLocalPlayerController(this))
	{
		if (EJewelBashMoveType::Type* MoveType = LastMoves.Find(PC))
		{
			return *MoveType;
		}
	}
	return EJewelBashMoveType::MT_None;
}

int32 AGrid::GetScoreMultiplierForMove_Implementation(EJewelBashMoveType::Type LastMoveType)
{
